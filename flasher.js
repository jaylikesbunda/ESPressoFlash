...
let ghostEspPopulateRequestId = 0;
        ...
        const GHOST_ESP_MANIFEST_BRANCH = 'Development-deki';
        ...
        async function populateGhostEspDropdown(owner, repo, fileExtension = '.zip', filterChip = null) {
            ...
            try {
                if (ghostEspSource === 'cdn') {
                    await populateGhostEspFromCDN(selectElement, fileExtension, filterChip, requestId);
                } else {
        ...
        async function populateGhostEspFromCDN(selectElement, fileExtension, filterChip, requestId) {
            const manifestUrl = ghostEspReleaseType === 'stable'
                ? GHOST_ESP_CDN_STABLE_URL
                : GHOST_ESP_CDN_PRERELEASE_URL;

            espLoaderTerminal.writeLine(`Fetching GhostESP ${ghostEspReleaseType} manifest from CDN...`);

            if (ghostEspStatusElem) {
                ghostEspStatusElem.textContent = `Fetching ${ghostEspReleaseType} manifest from CDN...`;
                ghostEspStatusElem.className = 'form-text mt-2 loading';
            }

            let response = await fetch(CF_WORKER_PROXY + encodeURIComponent(manifestUrl), { cache: 'no-store' });
            if (!response.ok && ghostEspReleaseType === 'stable') {
                espLoaderTerminal.writeLine(`Stable manifest not available (${response.status}), falling back to prerelease...`);
                response = await fetch(CF_WORKER_PROXY + encodeURIComponent(GHOST_ESP_CDN_PRERELEASE_URL), { cache: 'no-store' });
            }
            if (!response.ok) {
                throw new Error(`CDN manifest fetch failed: ${response.status} ${response.statusText}`);
            }

            const manifest = await response.json();
            if (requestId !== ghostEspPopulateRequestId) return;

            const commitEntries = Object.values(manifest?.commits || {});
            if (commitEntries.length === 0) {
                espLoaderTerminal.writeLine('⚠️ CDN manifest contains no commits.');
                selectElement.innerHTML = `<option value="">No releases found in manifest</option>`;
                return;
            }

            commitEntries.sort((a, b) => {
                const aDate = new Date(a?.updated_at || 0).getTime();
                const bDate = new Date(b?.updated_at || 0).getTime();
                return bDate - aDate;
            });

            const latestEntry = commitEntries[0];
            const configs = Array.isArray(latestEntry?.configs) ? latestEntry.configs : [];

            let optionsAdded = false;
            const channel = ghostEspReleaseType;

            for (const cfg of configs) {
                const zipName = cfg?.zip_name;
                if (!zipName || !zipName.endsWith(fileExtension)) continue;
                if (cfg?.channel && cfg.channel !== channel) continue;

                if (filterChip) {
                    const assetTarget = ghostEspZipToTarget[zipName];
                    const mappedChip = ghostEspChipMapping[assetTarget];
                    if (mappedChip !== filterChip) continue;
                }

                const cdnUrl = typeof cfg?.download_url === 'string' ? cfg.download_url.trim() : '';
                const gitlabMirror = (Array.isArray(cfg?.mirrors) && cfg.mirrors.length > 0)
                    ? cfg.mirrors[0].url : '';

                if (!cdnUrl && !gitlabMirror) continue;

                if (zipName === "esp32-generic.zip") {
                    const urlData1 = JSON.stringify({ cdn: cdnUrl, mirror: gitlabMirror });
                    const option1 = document.createElement('option');
                    option1.value = urlData1;
                    option1.textContent = "Generic ESP32";
                    selectElement.appendChild(option1);

                    const urlData2 = JSON.stringify({ cdn: cdnUrl, mirror: gitlabMirror });
                    const option2 = document.createElement('option');
                    option2.value = urlData2;
                ...
                const urlData = JSON.stringify({ cdn: cdnUrl, mirror: gitlabMirror });
                const option = document.createElement('option');
                option.value = urlData;
                option.textContent = ghostEspNiceNames[zipName] || zipName;
            ...
            if (optionsAdded) {
                selectElement.disabled = false;
                espLoaderTerminal.writeLine(`Loaded ${channel} builds from CDN: ${latestEntry.last_tag}`);
                if (ghostEspStatusElem) {
                    ghostEspStatusElem.textContent = `CDN ${channel}: ${latestEntry.last_tag}`;
                    ghostEspStatusElem.className = 'form-text text-success mt-2 success';
                }
            } else {
                let message = `No ${channel} builds found in CDN manifest`;
                if (filterChip) message += ` for ${filterChip}`;
                espLoaderTerminal.writeLine(`⚠️ ${message}`);
                selectElement.innerHTML = `<option value="">${message}</option>`;
                if (ghostEspStatusElem) {
                    ghostEspStatusElem.textContent = message;
        ...
        async function populateGhostEspFromGitHub(selectElement, owner, repo, fileExtension, filterChip, requestId) {
            if (!ghostEspStableReleases && !ghostEspPrereleases) {
                ...
                try {
                    espLoaderTerminal.writeLine(`Fetching GhostESP manifest from GitHub...`);
                    const manifestResponse = await fetch(GHOST_ESP_MANIFEST_URL, { cache: 'no-store' });
                    if (manifestResponse.ok) {
                        ...
                        const configToAsset = (config) => {
                            const zipName = config?.zip_name;
                            if (!zipName || !zipName.endsWith(fileExtension)) return null;

                            let downloadUrl = typeof config?.download_url === 'string' ? config.download_url.trim() : '';
                            const directZipUrl = /\.zip($|\?)/i.test(downloadUrl);

                            if (!directZipUrl) {
                                const releaseTag = config?.release_tag || latestEntry?.last_tag;
                                if (releaseTag) {
                                    downloadUrl = `https://github.com/${owner}/${repo}/releases/download/${releaseTag}/${zipName}`;
                                } else {
                                    downloadUrl = '';
                                }
                            }
                            ...
                            return { name: zipName, browser_download_url: downloadUrl };
                        };

                        const stableAssets = configs.filter(c => c?.channel === 'stable').map(configToAsset).filter(Boolean);
                        const prereleaseAssets = configs.filter(c => c?.channel === 'prerelease').map(configToAsset).filter(Boolean);
                        const commitAssets = configs.filter(c => c?.channel === 'commit').map(configToAsset).filter(Boolean);

                        const selectedStableAssets = stableAssets.length > 0 ? stableAssets : commitAssets;
                        const selectedPrereleaseAssets = prereleaseAssets.length > 0 ? prereleaseAssets : commitAssets;

                        if (selectedStableAssets.length > 0) {
                            ghostEspStableReleases = {
                                tag_name: stableAssets.length > 0 ? (latestEntry?.last_tag || 'stable') : `${GHOST_ESP_MANIFEST_BRANCH} (commit manifest)`,
                                assets: selectedStableAssets,
                            };
                        }
                        if (selectedPrereleaseAssets.length > 0) {
                            ghostEspPrereleases = {
                                tag_name: prereleaseAssets.length > 0 ? (latestEntry?.last_tag || 'prerelease') : `${GHOST_ESP_MANIFEST_BRANCH} (commit manifest)`,
                                assets: selectedPrereleaseAssets,
                            };
                        }

                        loadedFromManifest = !!(ghostEspStableReleases || ghostEspPrereleases);
                        if (loadedFromManifest) {
                            espLoaderTerminal.writeLine('Loaded GhostESP options from firmware manifest.');
                        }
                    }
                } catch (manifestError) {
                    console.warn('[Debug] GhostESP manifest fetch failed, falling back to releases API:', manifestError);
                }
        ...
        async function loadPwnPowerZip(zipUrl) {
            ...
            const proxyUrl = CF_WORKER_PROXY + encodeURIComponent(zipUrl);
        ...
        */ // <<< --- End of commented out block ---
        ...
        // --- Event Listener for GhostESP Source Toggle (CDN / GitHub) ---
        const ghostEspSourceBtns = document.querySelectorAll('.ghostesp-source-btn');
        ghostEspSourceBtns.forEach(btn => {
            btn.addEventListener('click', () => {
                const source = btn.getAttribute('data-source');
                console.log(`[Debug] GhostESP source toggle clicked: ${source}`);

                ghostEspSourceBtns.forEach(b => b.classList.remove('active'));
                btn.classList.add('active');

                ghostEspSource = source;
                ...
                ghostEspStableReleases = null;
                ghostEspPrereleases = null;

                if (ghostEspDownloadSection && !ghostEspDownloadSection.classList.contains('d-none')) {
                    populateGhostEspDropdown(GHOST_ESP_OWNER, GHOST_ESP_REPO, '.zip', selectedDevice)
                        .catch(err => {
                            console.error('[Debug] Error repopulating GhostESP after source toggle:', err);
                        });
                }
            });
        });
        ...
        async function loadGhostEspZip(optionValue) {
            ...
            try {
                ...
                if (ghostEspSource === 'cdn') {
                    zipBlob = await fetchGhostEspFromCDN(optionValue);
                } else {
        ...
        async function fetchGhostEspFromCDN(optionValue) {
             let urls;
            try {
                urls = JSON.parse(optionValue);
            } catch (_) {
                throw new Error("Invalid CDN option data.");
            }

            const cdnUrl = urls.cdn || '';
            const mirrorUrl = urls.mirror || '';

            if (ghostEspStatusElem) {
                ghostEspStatusElem.textContent = 'Fetching ZIP from CDN...';
                ghostEspStatusElem.className = 'form-text mt-2 loading';
            }

            if (cdnUrl) {
                espLoaderTerminal.writeLine(`Fetching GhostESP firmware from CDN: ${cdnUrl}`);
                try {
                    const proxyUrl = CF_WORKER_PROXY + encodeURIComponent(cdnUrl);
                    const response = await fetch(proxyUrl);
                    if (response.ok) {
                        const blob = await response.blob();
                        if (blob.size > 0) return blob;
                    }
                    espLoaderTerminal.writeLine(`CDN fetch failed (${response.status}), trying GitLab mirror...`);
                } catch (cdnError) {
                    espLoaderTerminal.writeLine(`CDN fetch error: ${cdnError.message}, trying GitLab mirror...`);
                }
            }

            if (mirrorUrl) {
                if (ghostEspStatusElem) {
                    ghostEspStatusElem.textContent = 'Fetching ZIP from GitLab mirror...';
                }
                espLoaderTerminal.writeLine(`Fetching GhostESP firmware from GitLab mirror: ${mirrorUrl}`);
                try {
                    const proxyUrl = CF_WORKER_PROXY + encodeURIComponent(mirrorUrl);
                    const response = await fetch(proxyUrl);
                    if (response.ok) {
                        const blob = await response.blob();
                        if (blob.size > 0) return blob;
                    }
                    throw new Error(`GitLab mirror fetch failed: ${response.status}`);
                } catch (mirrorError) {
                    throw new Error(`Both CDN and GitLab mirror failed. Last error: ${mirrorError.message}`);
                }
            }

            throw new Error("No download URL available for the selected build.");
        }
        ...
        async function fetchGhostEspFromGitHub(zipUrl) {
            ...
            const response = await fetch(proxyUrl);
            if (!response.ok) {
                let errorDetail = `Proxy fetch failed: ${response.status}`;
                try {
                    const errorText = await response.text();
